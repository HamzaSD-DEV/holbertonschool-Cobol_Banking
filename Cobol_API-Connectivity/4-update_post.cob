       ...
       01 API-URL          PIC X(50) VALUE "https://jsonplaceholder.typicode.com/posts/1".
       01 JSON-PAYLOAD     PIC X(200) VALUE "{"
                                       & " ""id"": 1,"
                                       & " ""title"": ""Updated COBOL Post"","
                                       & " ""body"": ""The body content has been updated from a COBOL program."","
                                       & " ""userId"": 1 "
                                       & "}".
       ...
       PROCEDURE DIVISION.
           ...
           IF SYSTEM-STATUS = 0
               DISPLAY "Curl command executed successfully. Checking response..."
               PERFORM READ-STATUS-CODE
               IF HTTP-STATUS-CODE >= 200 AND HTTP-STATUS-CODE < 300
                   DISPLAY "PUT request successful! HTTP Status: " HTTP-STATUS-CODE
                   PERFORM READ-RESPONSE-BODY
                   PERFORM PARSE-JSON-RESPONSE
                   DISPLAY "Successfully updated post with ID: " Parsed-ID
               ELSE
                   DISPLAY "PUT request failed. HTTP Status: " HTTP-STATUS-CODE
               END-IF
           ELSE
               DISPLAY "Error executing curl command. System Status: " SYSTEM-STATUS
           END-IF.
           STOP RUN.
